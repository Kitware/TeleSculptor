...
class KRTD
	...
	def fov_x
		return 2 * Math.atan((@x_dim / 2.0) / @focal_length_x)
	end

	def fov_y
		return 2 * Math.atan((@y_dim / 2.0) / @focal_length_y)
	end
...
def from_file(fp)
    ...
    return KRTD.new(focal_length_mat, rotation_mat, translation_vec, distortion)
end