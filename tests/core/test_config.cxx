...
{
  if (maptk::config::block_sep.size() != 1)
  {
...
IMPLEMENT_TEST(get_value_type_mismatch)
{
  ...
  EXPECT_EXCEPTION(maptk::bad_configuration_cast_exception,
...
IMPLEMENT_TEST(subblock)
{
  ...
  config->set_value(other_block_name + maptk::config::block_sep + keyc, valuec);
...
IMPLEMENT_TEST(subblock_nested)
{
  ...
  maptk::config_t const subblock = config->subblock(nested_block_name);
...
IMPLEMENT_TEST(subblock_view)
{
  ...
  if (config->has_value(block_name + maptk::config::block_sep + keyb))
  {
...
IMPLEMENT_TEST(subblock_view_nested)
{
  ...
  maptk::config_t const subblock = config->subblock_view(nested_block_name);
